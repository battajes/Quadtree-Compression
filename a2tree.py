...
class QuadTreeNodeInternal(QuadTreeNode):
    ...
    def convert_to_pixels(self, width: int, height: int) -> List[List[int]]:
        ...
        bottom_left = self.children[0].convert_to_pixels(width // 2, height // 2)
        bottom_right = self.children[1].convert_to_pixels(width - (width // 2),
                                                          height // 2)
        top_left = self.children[2].convert_to_pixels(width // 2,
                                                      height - (height // 2))
        top_right = self.children[3].convert_to_pixels(width - (width // 2),
                                                       height - (height // 2))

        for i in range(len(bottom_left)):
            bottom_left[i].extend(bottom_right[i])
        for i in range(len(top_left)):
            top_left[i].extend(top_right[i])
    ...
    def restore_from_preorder(self, lst: List[str], start: int) -> int:
        ...
        for i in range(4):
            if lst[start] == 'E':
                ...
                start += self.children[i].restore_from_preorder(lst, start)
            else:
    ...
    def mirror(self) -> None:
        ...
        """
        # TODO
        if isinstance(self, QuadTreeNodeInternal):
            self.children[2] = deepcopy(self.children[0])
            self.children[3] = deepcopy(self.children[1])
            if isinstance(self.children[2], QuadTreeNodeInternal):
                self.children[2].mirror()
            if isinstance(self.children[3], QuadTreeNodeInternal):
...
class QuadTree:
    ...
    def _build_tree_helper(self, pixels: List[List[int]]) -> QuadTreeNode:
        ...
        if len(pixels) == 1 and len(pixels[0]) == 1:
            return QuadTreeNodeLeaf(pixels[0][0])
        stand_dev, mean = standard_deviation_and_mean(pixels)
        if stand_dev <= self.loss_level:
            return QuadTreeNodeLeaf(int(mean))
        else:
            new = self._split_quadrants(pixels)
            node_ = QuadTreeNodeInternal()
            for i in range(4):
                node_.children[i] = self._build_tree_helper(new[i])
            return node_
    ...
    def _split_quadrants(pixels: List[List[int]]) -> List[List[List[int]]]:
        ...
        bottom_left = [s[:width // 2] for s in pixels[:height // 2]]
        bottom_right = [s[width // 2:] for s in pixels[:height // 2]]
        top_left = [s[:width // 2] for s in pixels[height // 2:]]
        top_right = [s[width // 2:] for s in pixels[height // 2:]]
...
def maximum_loss(original: QuadTreeNode, compressed: QuadTreeNode) -> float:
    ...
    max_loss = 0
    if isinstance(original, QuadTreeNodeEmpty) or isinstance(compressed,
                                                             QuadTreeNodeEmpty):
        return 0
    if isinstance(original, QuadTreeNodeLeaf) and isinstance(compressed,
                                                             QuadTreeNodeLeaf):
        return abs(compressed.value - original.value)
    if isinstance(original, QuadTreeNodeInternal) and isinstance(
            compressed, QuadTreeNodeLeaf):

        for i in range(4):
            x = compressed.convert_to_pixels(original.children[i].tree_size(),
                                             compressed.value)
            stand_dev, y = standard_deviation_and_mean(x)
            max_loss = max(max_loss, int(stand_dev))
        if isinstance(original, QuadTreeNodeInternal) and isinstance(
                compressed, QuadTreeNodeInternal):
            for i in range(4):
                if maximum_loss(original.children[i], compressed.children[i]):
                    max_loss = max(max_loss, int(maximum_loss(original.children[i],
